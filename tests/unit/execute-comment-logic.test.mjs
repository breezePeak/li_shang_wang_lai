...
describe('comments:execute refactored logic', () => {
  ...
  it('按最近采集时间排序并优先返回最近采集的评论，不让旧评论挤占 max-count', () => {
    ...
    const rows = listPendingCommentsGroupedByHomepageAndWork({ limit: 1 });
  ...
  it('isReplyTextTooShort 会按长度和安全规则判断已有回复是否需要重写', () => {
    ...
    expect(isReplyTextTooShort('2222', { minLength: 1 })).toBe(true);
    ...
    expect(isReplyTextTooShort('Hermes代看后觉得2222这条反馈需要再看', { minLength: 15 })).toBe(false);
  ...
  it('parseArgs 支持 --max-count 作为 --limit 别名', () => {
    const args = parseArgs(['--max-count', '2']);
    expect(args.limit).toBe(2);
  });
