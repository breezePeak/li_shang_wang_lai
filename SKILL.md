...
| "看看好友回访候选"、"哪些朋友点赞了" | visits prepare，仅预览 |
...
```bash
npm run interactions:scan -- --type comment    # 扫描评论，入库去重
npm run interactions:scan -- --type like       # 扫描点赞通知
npm run interactions:scan -- --type all        # 同时扫描评论和通知
```
...
# 步骤 2：用户填写 replyText + 确认后，dry-run 定位目标评论
npm run comments:reply -- --plan <计划文件> --dry-run

# 步骤 3：用户再次确认后，真实发送（最多 1 条）
npm run comments:reply -- --plan <计划文件> --execute --max-items 1
```

> **待新增命令**（S2-S3）：`actions:pending`、`comments:prepare`、`actions:approve`、`comments:execute`。届时审批流程将从手工编辑 JSON 升级为命令式对话。
...
npm run interactions:scan -- --type comment
  ↓
Skill 展示待处理评论摘要（昵称、作品、评论内容）
  ↓
用户指定回复某条评论
  ↓
npm run comments:plan  → 生成计划文件
  ↓
用户编辑 replyText 并确认
  ↓
...
用户说 "确认"
  ↓
npm run comments:reply -- --plan <计划> --dry-run
  ↓
dry-run 成功 → 告知用户定位成功
...
用户说 "发送"
  ↓
npm run comments:reply -- --plan <计划> --execute --max-items 1
  ↓
成功 → 告知用户已发送 + 审计记录已保存
失败 → 报告原因 + 证据路径
```
...
- 评论审批仍依赖手工编辑 JSON 计划文件（S2-S3 将改为命令式审批）；
- 好友回访仅提供候选预览，真实点赞代码层硬阻断；
- 部分命令尚未支持 `--json` 输出（S1.5 接入中）；
- `actions:pending` 待处理报告命令尚未新增（S2）。
