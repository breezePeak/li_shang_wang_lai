...
| 填写回复内容 | **Agent 操作** | Agent 读取扫描 JSON，根据评论内容、作品上下文和安全规则，生成并填写每条评论的 `reply_text` |
| 写入数据库 | CLI: `comments:prepare` | 将 Agent 已填写的 `reply_text` 写入 `work_comments`（不改变 `reply_status`） |
| 执行评论回复 | CLI: `comments:execute` | 读取 JSON 执行回复；默认真实执行 |
| 准备回访 | CLI: `return-visit:prepare` | 用户明确要求回访时，消费待回访 JSON 并生成回访任务 |
