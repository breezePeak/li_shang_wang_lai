...
| 执行评论回复 | `comments:execute` | 读取同一个 JSON 执行回复；带 `--execute` 才真实发送 |
...
| 评论回复 | `interactions:scan -- --type comment --days N --max-count M --generate-reply-json` | 填写 `reply_text` 后执行 `comments:prepare` 和 `comments:execute` |
...
`N` 和 `M` 由 Agent 从用户要求或安全默认值决定。默认建议 `days=7`、`max-count=100`；真实执行动作仍必须显式使用 `--execute`。
