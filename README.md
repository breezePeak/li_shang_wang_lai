...
| 好友点赞回访计划 | 初版已实现 | 通知面板点头像进入主页，查找最新视频 |
| 好友作品回赞执行 | 初版已实现 | 通知面板逐条处理，点赞并记录 |
...
> 点赞回访功能已实现基础流程（通知面板点头像 → 主页找视频 → 点赞记录），但仍在验证中，建议先小范围 dry-run 测试。
...
| `npm run likes:plan` | 生成好友回赞计划（通知面板扫描） | 初版可用 |
| `npm run likes:reciprocate` | 执行好友回赞 | 验证阶段 |
