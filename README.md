...
Agent 在接到用户请求后，应**主动引导用户完成以下步骤**，不要跳过：

1. **确认用户已登录**：提醒用户先执行 `npm run auth` 完成浏览器扫码登录
2. **引导用户执行扫描**：根据用户意图引导用户执行 `npm run interactions:scan`（如 `--type all --days 1`）
3. **等待用户确认结果**：扫描完成后，用户会提供输出日志，Agent 再根据结果继续后续动作

Agent 不应在用户尚未完成登录和扫描前就尝试执行回复或回访。
