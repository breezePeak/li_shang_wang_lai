...
async function collectCommentsFromNotificationWork(page, n, { sourceEventId, notificationDays, notificationHours, dedupeContext, runtimeCollectedWorkKeys }) {
  ...
  const workResult = upsertWorkContext({
    ...
    workDesc: context.workDesc || n.workDesc || null,
    ...
    if (apiResult.ok && apiResult.comment) {
      ...
      if (!dedupeContext.commentKeys.has(dedupeKey)) {
        const result = upsertWorkComment({
          ...
          eventCreatedAt: c.eventCreatedAt || null,
          commentKey,
          sourceEventId,
...
async function runNotificationScan(page, run, type, pauseAfterOpen = 0, debugNotificationDom = false, scanPlan = {}) {
  ...
  async function processNoticeApiItem(item, notificationIndex) {
    ...
    const result = upsertNotificationEvent({
      ...
      eventCreatedAt: normalized.eventCreatedAt || null,
      fingerprint: fp,
    ...
    upsertWorkContext({
      ...
      workDesc: normalized.workDesc || null,
...
async function main() {
  ...
  try {
    ...
    const ctx = await createBrowserContext({ headless: options.headless, enableReuse: options.keepOpen });
    browser = ctx.browser;
