...
export async function promptRecoveryAction(step, code, message, evidenceDir) {
  ...
  console.log(`  错误步骤: ${step}`);
  console.log(`  信息: ${message}`);
  console.log('');
  console.log(`  现场证据已保存: ${evidenceDir}`);
  console.log('');
  console.log('  浏览器已保持打开。你可以手动进入正确页面后重新检测。');
  console.log('');
  console.log('  请选择：');
  console.log('    [r] 重新检测当前页面');
  console.log('    [s] 跳过当前步骤');
  console.log('    [d] 再保存一次诊断信息');
  console.log('    [q] 安全退出并关闭浏览器');
  console.log('    [k] 安全退出但保持浏览器打开');
  ...
  for (;;) {
    try {
      ...
      switch (answer) {
        ...
        default:
          console.log('  无效选择，请输入 r/s/d/q/k');
      }
    } catch {
